...
const MultipleChoiceMultiple = async () => {
  const cookieStore = await cookies();

  let initialData;
  try {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/v1/practice/listening/multipleChoiceMultiple?page=1&limit=10`,
      {
      ...
      <div className="container mx-auto max-w-4xl px-4 py-8 sm:px-6">
        <div className="mb-8">
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Listen and select all correct responses — more than one answer required.
          </p>
        </div>
        <FilterQuestions2
          apiPath="/api/v1/practice/listening/multipleChoiceMultiple"
