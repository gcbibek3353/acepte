...
const WriteFromDictationPage = async () => {
  const cookieStore = await cookies();

  let initialData;
  try {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/v1/practice/listening/writeFromDictation?page=1&limit=10`,
      {
    ...
    <div className="min-h-screen bg-background">
      <div className="container mx-auto max-w-4xl px-4 py-8 sm:px-6">
        <div className="mb-8">
          ...
          <h1 className="text-3xl font-bold tracking-tight text-foreground">Write From Dictation</h1>
          <p className="mt-1 text-sm text-muted-foreground">
            Listen to the sentence and type it exactly as you hear it.
          </p>
        </div>
        <FilterQuestions2
          apiPath="/api/v1/practice/listening/writeFromDictation"
...
export default WriteFromDictationPage;
