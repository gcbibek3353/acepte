                ...
                <div className="space-y-2">
                  {selectedWords.map((sw) => {
                    ...
                    return (
                      <div
                        key={sw.position}
                        className={`flex items-center justify-between p-3 rounded-lg border ${isCorrectlyFlagged ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'}`}
                      >
                        <span className={`font-semibold ${isCorrectlyFlagged ? 'text-green-800' : 'text-red-800'}`}>
                          {sw.word}
                          <span className="ml-2 text-xs font-normal text-gray-500">(position {sw.position})</span>
                        </span>
                        <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${isCorrectlyFlagged ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
