...
interface Props {
  answers: ListeningHiwDetail['answers']
  incorrectWords: ListeningHiwDetail['incorrectWords']
}

type SelectedWord = { word: string; position: number }

const Listening_hiw_answer = ({ answers, incorrectWords }: Props) => {
  if (answers.length === 0) {
    return (
    ...
    <div className="mt-6 rounded-lg border border-border bg-card shadow-sm overflow-hidden">
      ...
      <div className="divide-y divide-border">
        {answers.map((answer, index) => {
          const selectedWords = (answer.selectedWords as SelectedWord[]) ?? []
              ...
              {selectedWords.length === 0 ? (
                ...
                <div className="space-y-2">
                  {selectedWords.map((sw) => {
                    const isCorrectlyFlagged = incorrectPositions.has(sw.position)
                    return (
                      <div
                        key={sw.position}
                        className={`flex items-center justify-between p-3 rounded-lg border text-sm ${isCorrectlyFlagged ? 'bg-green-50 border-green-200 dark:bg-green-950/20 dark:border-green-900/40' : 'bg-red-50 border-red-200 dark:bg-red-950/20 dark:border-red-900/40'}`}
                      >
                        <span className={`font-semibold ${isCorrectlyFlagged ? 'text-green-800 dark:text-green-300' : 'text-red-800 dark:text-red-300'}`}>
                          {sw.word}
                          <span className="ml-2 text-xs font-normal text-muted-foreground">(pos. {sw.position})</span>
                        </span>
                        <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${isCorrectlyFlagged ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' : 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400'}`}>
                          {isCorrectlyFlagged ? 'Correctly flagged' : 'Wrongly flagged'}
                        </span>
                      </div>
                    )
                  })}
...
export default Listening_hiw_answer
